...
def send_email_command():
    ...
    ep.create_log_file("Alice", from_user, to_user, str1, hashed_val, '', '', '')

def send_reverse_email_command():
    from_user = from_guess_input.get()
    to_user = to_guess_input.get()
    ...
    se.send_email(from_user, to_user, subject, body, smtp_server, smtp_port, smtp_user, smtp_password)
    ep.create_log_file("Bob", from_user, to_user, '', '', coin_flip_val, '', '')
...
def send_reveal_values_command():
    ...
    se.send_email(from_user, to_user, subject, body, smtp_server, smtp_port, smtp_user, smtp_password)
    ep.create_log_file("Alice 2", from_user, to_user, str1, '', '', str2, coin_flip_val)
...
reverse_password_label.grid(row=4, column=0)
reverse_password_input = tk.Entry(guess_frame, show='*')
reverse_password_input.grid(row=4, column=1)

# Coin flip value
coin_flip_label_guess = tk.Label(guess_frame, text="Enter coin flip value: ")
coin_flip_label_guess.grid(row=5, column=0, sticky='w')
coin_flip_input_guess = ttk.Combobox(guess_frame, values=['head', 'tail'], state='readonly')
coin_flip_input_guess.grid(row=5, column=1, sticky='w')

# Create a button to send the pguessed value
guess_value_button = tk.Button(guess_frame, text='send value guess', command=send_reverse_email_command)
guess_value_button.grid(row=6, column=0)
